...
export class AdminProductFormView {
    renderForm(products: Product| null, categories: Category[]): string{
        if (products) {
            ...
            <div class="row">
              ...
              <div class="col-lg-8">
                ...
                ${categories.map((c:Category) => `<option value="${c.id}" ${c.id == products.category_id ? 'selected' : ''}>${c.name}</option>`).join('')}  
                </select>
                        </div>
                      </div>
