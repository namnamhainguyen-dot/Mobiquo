...
export class ProductService extends ApiService {
    async getAll(): Promise<Product[]> {
        ...
        const data: any[] = await this.get<any[]>('/products');
    ...
    async getById(id: string): Promise<Product> {
        const data: any = await this.get<any>(`/products/${id}`);
    ...
    async create(p: Product | any): Promise<Product> {
        // Loại bỏ ID cũ để MongoDB tự tạo mới
        const { id, ...dataToSend } = p; 
        return await this.post<Product>('/products', dataToSend);
    }

    async edit(p: Product | any): Promise<Product> {
        // Lấy ID từ object để làm URL, phần còn lại gửi vào body
        const { id, ...dataToSend } = p;
        return await this.update<Product>(`/products/${id}`, dataToSend);
    }
    ...
    async search(query: string, catId: string, brandId: string): Promise<Product[]> {
