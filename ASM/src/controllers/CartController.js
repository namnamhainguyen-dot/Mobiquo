...
export class CartController {
    ...
    attachEvents() {
        ...
        document.querySelector('#order-form')?.addEventListener('submit', async (e) => {
            ...
            const items = this.cartService.getItems();
            const total = this.cartService.getTotalPrice();
            let res = await this.orderService.create(new Order(undefined, new Date(), total, new User("", name, phone, "", "", address, "user"), items, 'pending'));
            if (await res) {
                ...
                alert("Có lỗi xảy ra trong quá trình đặt hàng. Vui lòng ktr và thử lại");
            }
        });
    }
}
